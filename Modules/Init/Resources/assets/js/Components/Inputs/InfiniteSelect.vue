...
export default defineComponent({
    ...
    mounted() {
        this.paginatedObject.meta.per_page = parseInt(this.perPage) || 15;
    ...
    methods: {
        ...
        async fetchResults(query, loading, more = false) {
            ...
            let params = {};
            if (vm.paginatedObject?.meta?.current_page && more) {
                if (!vm.paginatedObject?.links?.next) {
                    return false;
                }
                params.page = vm.paginatedObject.meta.current_page + 1;
                params.per_page = vm.paginatedObject.meta.per_page;
            }
            if (vm.searchQuery) {
    ...
    computed: {
        hasNextPage() {
            let vm = this;
            // return vm.paginatedObject;
            return (
                (vm.paginatedObject.meta?.current_page &&
                    vm.paginatedObject.links.next) ||
