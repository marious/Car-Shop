<template>
    ...
    >
        <div
            class="vue-editable-cell__handle"
            v-if="!editing"
            v-show="!dragging"
            @click.stop
            v-dragged="$_onDragged"
        ></div>
        <div
            class="vue-editable-cell__drag-box"
            :style="dragBoxStyle"
            v-if="dragging"
        ></div>
        <select
            class="vue-editable-cell__select"
            v-if="editing && Array.isArray(options)"
            :value="value"
    ...
    computed: {
        ...
        wrapperStyle() {
            return {
                ...
                top: `${
                    this.target.offsetTop + this.target.offsetParent.offsetTop
                }px`,
                left: `${
                    this.target.offsetLeft + this.target.offsetParent.offsetLeft
                }px`,
            };
        },
        inputStyle() {
        ...
        dragBoxStyle() {
            ...
            return {
                width: `${this.targetWidth}px`,
                height: `${
                    this.targetHeight + this.targetHeight * this.dragSteps
                }px`,
                top:
                    this.dragOffset <= 0
                        ? `-${2 + this.targetHeight * this.dragSteps}px`
                        : "-2px",
                left: "-2px",
            };
        },
        ...
        rowIndex() {
            if (!this.target.closest("tr") || !this.targetContainer)
                return null;
            return Array.from(this.targetContainer.rows).indexOf(
                this.target.closest("tr")
            );
        },
        targetContainer() {
            if (this.target.closest("tbody"))
                return this.target.closest("tbody");
            if (this.target.closest("table"))
        ...
        maxDragSteps() {
            if (!this.dragOffset || this.rowIndex === null || !this.rowsCount) {
                return 0;
            }
